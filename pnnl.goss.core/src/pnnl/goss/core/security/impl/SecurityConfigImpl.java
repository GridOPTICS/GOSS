...
@Component(service = SecurityConfig.class, configurationPid = "pnnl.goss.security", configurationPolicy = ConfigurationPolicy.REQUIRE)
public class SecurityConfigImpl implements SecurityConfig {
    ...
    public void activate(Map<String, Object> properties) {
        log.info("Activating SecurityConfigImpl");
        updated(properties);
    }
    ...
    public synchronized void updated(Map<String, Object> properties) {
        if (properties != null) {
            ...
            // create system realm
            managerUser = getProperty(SecurityConstants.PROP_SYSTEM_MANAGER, null);
            managerPassword = getProperty(SecurityConstants.PROP_SYSTEM_MANAGER_PASSWORD, null);
            ...
            String useTokenString = getProperty(SecurityConstants.PROP_SYSTEM_USE_TOKEN, null);
            if (useTokenString != null && useTokenString.trim().length() > 0) {
                try {
                    this.useToken = Boolean.parseBoolean(useTokenString);
                } catch (Exception e) {
    ...
    public JWTAuthenticationToken parseToken(String token) {
        try {
            ...
            String jsonToken = payload.toString();
